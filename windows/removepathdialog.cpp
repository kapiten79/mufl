...
RemovePathDialog::RemovePathDialog(QWidget *parent):QDialog (parent)
{
    try {
        ...
        qDebug() << "Исключение возникло в методе RemovePathDialog класса RemovePathDialog " << error.what();
    }
}
...
void RemovePathDialog::removePathRecursive(const QString& rmPath)
{
    try {
        ...
        qDebug() << "Исключение возникло в методе removePathRecursive класса RemovePathDialog " << error.what();
    }
